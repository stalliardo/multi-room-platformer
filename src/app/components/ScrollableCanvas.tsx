...
const ScrollableCanvas = () => {
  ...
  useEffect(() => {
        ...
        enterDoor: {
          ...
          onComplete: () => {
            gsap.to(overlay, {
              ...
              onComplete: () => {
                level++;
                if(level === 5) level = 1; // TODO remove looping
                levels[level].init();
    ...
    let levels: Levels = {
      ...
      4: {
        init: () => {
          collisionBlocks = getCollisionBlocksArray("level4");
          background = new Sprite({ position: { x: 0, y: 0 }, imageSrc: "backgroundLevel4.png" });
          player.collisionBlocks = collisionBlocks;
          if(player.currentAnimation?.isActive) player.currentAnimation.isActive = false;
          player.position.x = 741;
          player.position.y = 162;
